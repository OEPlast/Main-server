...
const updateShipment = async (req: Request, res: Response) => {
  try {
    const { shipmentId } = req.params;
    const updates = req.body as { status?: unknown } & Record<string, unknown>;
    if ('status' in updates && !isShipmentStatus(updates.status)) {
      return res.status(400).json({ error: 'Invalid status value' });
    }
    const result = await ShipmentService.updateShipment(shipmentId, updates as Record<string, unknown>);
    return res.status(result.code).json({ message: result.message, data: result.data });
  } catch (error) {
...
const addTrackingUpdate = async (req: Request, res: Response) => {
  try {
    const { shipmentId } = req.params;
    const updateData = req.body as { status?: unknown; location?: string; description?: string };
    if (!isShipmentStatus(updateData.status)) {
      return res.status(400).json({ error: 'Invalid status value' });
    }
...
const trackShipment = async (req: Request, res: Response) => {
  try {
    const { shipmentId } = req.params;
    const result = await ShipmentService.trackShipment(shipmentId);
    return res.status(result.code).json({ message: result.message, data: result.data });
  } catch (error) {
