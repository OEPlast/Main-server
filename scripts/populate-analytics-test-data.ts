...
const STATES_NIGERIA = [
  ...
  'Ogun', 'Kaduna', 'Edo', 'Anambra', 'Enugu', 'Imo',
  'Plateau', 'Akwa Ibom', 'Ondo', 'Osun', 'Cross River', 'Abia',
  'Kwara', 'Benue', 'Bauchi', 'Taraba', 'Adamawa', 'Borno'
];
const LGAS_BY_STATE = {
  'Lagos': ['Ikeja', 'Surulere', 'Lekki', 'Victoria Island', 'Ikorodu', 'Badagry', 'Epe', 'Alimosho', 'Oshodi-Isolo', 'Agege'],
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.push({
      ...
      address: [{
        ...
        lastName,
        phoneNumber: `+234${randomInt(7000000000, 8999999999)}`,
        address1: `${randomInt(1, 500)} ${randomElement(['Allen Avenue', 'Victoria Street', 'Ahmadu Bello Way', 'Awolowo Road', 'Herbert Macaulay Way'])}`,
...
async function createTestCategories(count = 20) {
  ...
  const categoryNames = [
    ...
    'Furniture', 'Music', 'Pet Supplies', 'Office', 'Tools',
    'Jewelry', 'Baby', 'Outdoor', 'Gaming', 'Art'
  ];
...
async function createTestOrders(count = 1000) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const transactionStatus = status === 'Completed' ? 'completed' : 
  ...
  for (let i = 0; i < count; i++) {
    ...
    const exists = wishlistEntries.some(
      w => w.user.toString() === user._id.toString() && 
           w.product.toString() === product._id.toString()
    );
